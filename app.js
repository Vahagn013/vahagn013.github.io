...
function updateScoreOnServer(newScore) {
    ...
    fetch(`${SERVER}/update-score`, {
        ...
        alert('Could not connect to server to update score.');
...
function fetchScoreFromServer() {
    ...
    fetch(`${SERVER}/get-score?username=${encodeURIComponent(username)}`)
        ...
        .catch(err => {
            alert('Could not connect to server to fetch score.');
...
loginBtn.onclick = function() {
    ...
    if (!inputUser) return alert('Please enter a username');
    fetch(`${SERVER}/login`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
    ...
    .catch(err => {
        alert('Could not connect to server for login. Make sure the server is running and the address is correct.');
